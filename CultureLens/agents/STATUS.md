...
- 2026-08-06 知识包拆成 sidecar：`knowledge-pack.json` 只留 version/relations；看点列表进 `elements-sight`；现场介绍 / 主题 / 每语言 locales 各一文件；去掉主 JSON 内嵌 manifest（只用 `pack-manifest.json`）。`ContentRole` 区分看点与文化历史；识别 catalog 只收看点。
