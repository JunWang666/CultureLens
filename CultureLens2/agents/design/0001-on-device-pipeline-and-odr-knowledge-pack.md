...
- 状态：已实现（2026-08-02）
- 取代：CultureLens `design/0005`（Go BFF 承担识别管线）在 CultureLens2 中的对应部分

## 背景

CultureLens 的识别链路为 App → Go BFF →（PostgreSQL 候选 + prompt 拼接）→ Gemini。数据库内容仅 36KB，拼接逻辑为纯函数，全部可以在端侧完成；服务端只保留 LLM 出口与图片托管。CultureLens2 因此把管线搬到端侧，后端依赖降为两个外部服务。
