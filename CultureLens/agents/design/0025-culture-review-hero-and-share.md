...
1. **主图**：按行程对象顺序，取知识包 `element.introduction` 里第一张 HTTPS 图（`imageBlocks.first`）作为回顾主图；详情页与分享卡共用同一解析（`VisitTripHero`）。无图时回退海报感 `ObjectArtwork`（见 `0024`）。
...
4. **可选生图**：设置「图片生成」默认关闭；仅当开启且介绍无主图时，才调火山方舟 Seedream（`VolcengineImageClient`）生成封面。
