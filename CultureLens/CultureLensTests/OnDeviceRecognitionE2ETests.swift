...
///     xcodebuild ... test -only-testing:CultureLensTests/OnDeviceRecognitionE2ETests
struct OnDeviceRecognitionE2ETests {
  @MainActor @Test func endToEndRecognitionViaGateway() async throws {
    ...
    let repoRootFixture = URL(fileURLWithPath: #filePath)
      .deletingLastPathComponent() // CultureLensTests/
      .deletingLastPathComponent() // CultureLens/
