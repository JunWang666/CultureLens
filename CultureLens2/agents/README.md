# CultureLens2 agents 工作区

CultureLens2 是 CultureLens 的端侧化版本：知识库与 LLM prompt 拼接逻辑从 Go 后端搬到 iOS 端侧，App 只依赖两个外部资源——Cloudflare AI Gateway（LLM）与 Cloudflare R2（图片 URL）。
